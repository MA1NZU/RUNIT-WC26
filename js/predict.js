...
let jokerMatchId = null;    // which match has the joker this round
let jokerRoundId = null;    // which round the joker was used in
let currentRoundId = null;  // active round id
...
async function init() {
  ...
  // Store current round id (first active round)
  ...
  // Load predictions AND joker in parallel
  ...
  // Store joker state
  if (jokerRes.data) {
    jokerMatchId = jokerRes.data.match_id;
    jokerRoundId = jokerRes.data.round_id;
  }
...
function lockMatchCard(matchId) {
  ...
  if (saveBtn)   { saveBtn.disabled = true; saveBtn.textContent = '🔒 Locked'; }
...
function renderMatches(rounds, predMap) {
  ...
  const now       = new Date();
  const oneHour   = 60 * 60 * 1000;
  const jokerUsed = jokerMatchId !== null; // has joker been placed this round?
    ...
    <div class="card" style="margin-bottom:24px; display:flex; align-items:center;
      ...
      <div>
        ...
        <div style="font-size:0.85rem; color:var(--text-muted)">
          ${jokerUsed
            ? `Used on <strong style="color:var(--gold)">Match #${jokerMatchId}</strong> this round. Points doubled!`
            : 'Pick one match this round to double your points. Use it wisely!'}
        </div>
      </div>
      <span class="badge ${jokerUsed ? '' : 'badge-green'}"
            style="${jokerUsed
              ? 'background:rgba(255,215,0,0.15); color:var(--gold); border:1px solid rgba(255,215,0,0.3)'
  ...
  rounds.forEach(round => {
    ...
    round.matches.forEach(match => {
      ...
      // Joker state for this match
      const isJokerOnThisMatch = jokerMatchId === match.id;
      const canPlaceJoker      = !jokerUsed && !isLocked; // can still place joker
      ...
      let resultBadge = '';
      if (isFinished && pred) {
        const multiplier = isJokerOnThisMatch ? ' (🃏 ×2)' : '';
        if (pred.points_earned >= 12) {
          resultBadge = `<span class="result-badge exact">⚡ Perfect! +12pts 🃏</span>`;
        } else if (pred.points_earned === 6) {
          resultBadge = `<span class="result-badge exact">⚡ Perfect! +6pts${multiplier}</span>`;
        } else if (pred.points_earned === 6 && isJokerOnThisMatch) {
          resultBadge = `<span class="result-badge correct">✓ Correct +6pts 🃏</span>`;
        } else if (pred.points_earned === 3) {
          resultBadge = `<span class="result-badge correct">✓ Correct outcome +3pts${multiplier}</span>`;
        } else {
      ...
      if (isJokerOnThisMatch) {
        // Joker is on THIS match — show active state + remove option if not locked
        jokerHtml = `
        ...
        // Joker available and match is open — show place button
        jokerHtml = `
      ...
      const adminGoLive = isAdmin && !isFinished && isKickedOff ? `
        <div style="border-top:1px solid var(--border); margin-top:16px; padding-top:14px">
          ...
          <div style="display:flex; align-items:center; justify-content:center; gap:10px; flex-wrap:wrap">
            <input type="number" id="live-home-${match.id}" value="0" min="0" max="20"
                   style="width:56px; background:var(--dark); border:2px solid var(--green);
                          border-radius:8px; padding:8px; color:var(--text);
                          font-size:1.2rem; font-weight:800; text-align:center; outline:none">
            <span style="color:var(--text-muted); font-size:1.3rem; font-weight:800">–</span>
            <input type="number" id="live-away-${match.id}" value="0" min="0" max="20"
                   style="width:56px; background:var(--dark); border:2px solid var(--green);
                          border-radius:8px; padding:8px; color:var(--text);
                          font-size:1.2rem; font-weight:800; text-align:center; outline:none">
            <button class="btn btn-primary btn-sm" onclick="updateLiveScore(${match.id})">
      ...
      // ---- Joker glow border ----
      const jokerBorder = isJokerOnThisMatch
        ? 'border-color:var(--gold); box-shadow:0 0 12px rgba(255,215,0,0.15);'
        : '';
...
async function placeJoker(matchId, roundId) {
  if (!confirm('🃏 Use your joker on this match? This will DOUBLE your points for this match. You can remove it before kickoff.')) return;
...
async function removeJoker(matchId) {
  ...
  jokerMatchId = null;
  jokerRoundId = null;
  showToast('Joker removed. You can place it on another match.');
  setTimeout(() => location.reload(), 1000);
}
...
// ============================================
// ADMIN: UPDATE LIVE SCORE (joker-aware)
// ============================================
async function updateLiveScore(matchId) {
  ...
  // Get all jokers for this match (to know who doubled)
  ...
  for (const p of preds) {
    ...
    // Base points
    let basePoints = isPerfect ? 6 : isCorrect ? 3 : 0;

    // Double if joker was placed on this match by this user
    let newPoints = jokerUserIds.has(p.user_id) ? basePoints * 2 : basePoints;
